...
<template>
  <BaseModal
    ...
    :loading="loading"
    max-width="600"
    :title="`${actionText} unidad de transporte`"
    @accept="$emit('confirm', bus)"
    @cancel="$emit('cancel')"
    @close="$emit('cancel')"
  >
    <div class="text-center text-secondary font-weight-bold d-flex flex-column ga-5">
      <p>Esta accion cambiara el estado de la unidad de transporte a {{ targetStatusText }}</p>
      <p>&iquest;Desea {{ actionText.toLowerCase() }} la unidad de transporte?</p>
    </div>
  </BaseModal>
</template>
