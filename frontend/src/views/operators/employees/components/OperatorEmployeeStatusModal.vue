...
const rules = computed(() => ({
  motivo_desactivacion: {
    requiredIf: helpers.withMessage(
      'El motivo de desactivacion es requerido.',
...
<template>
  ...
  >
    <div class="text-center text-secondary font-weight-bold d-flex flex-column ga-5">
      <p>
        Esta accion cambiara el estado del empleado a {{ targetStatusText }}
      </p>
      <p>&iquest;Desea {{ actionText.toLowerCase() }} el empleado?</p>
    </div>
    ...
    >
      <div class="text-secondary font-weight-bold mb-1">
        Ingrese los motivos para cambiar el estado del empleado
      </div>
      <v-textarea
