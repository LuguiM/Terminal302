...
import {
  createOperatorRoute,
  deleteOperatorRoute,
  getAvailableOperatorRoutes,
  getOperatorRoutes,
  toggleOperatorRouteStatus,
} from '@/services/operatorRouteService'
import OperatorRouteCreateModal from '@/views/operators/routes/components/OperatorRouteCreateModal.vue'
import OperatorRouteDeleteModal from '@/views/operators/routes/components/OperatorRouteDeleteModal.vue'
import OperatorRouteStatusModal from '@/views/operators/routes/components/OperatorRouteStatusModal.vue'

const routes = ref([])
const availableRoutes = ref([])
const loading = ref(false)
const availableRoutesLoading = ref(false)
...
const fetchRoutes = async () => {
  ...
  try {
    ...
    routes.value = data.operador_rutas ?? []
...
const fetchAvailableRoutes = async () => {
  availableRoutesLoading.value = true

  try {
    const { data } = await getAvailableOperatorRoutes()
    availableRoutes.value = data.rutas ?? []
  } catch {
    availableRoutes.value = []
  } finally {
    availableRoutesLoading.value = false
  }
}
...
const handleCreateRoute = async (payload) => {
  ...
  try {
    const { data } = await createOperatorRoute(payload)
    notify.success(data.message || 'Ruta asignada correctamente.')
    closeModals()
    await fetchRoutes()
  } finally {
...
const handleToggleStatus = async () => {
  ...
  try {
    const { data } = await toggleOperatorRouteStatus(selectedRoute.value.id)
...
const handleDeleteRoute = async () => {
  ...
  try {
    const { data } = await deleteOperatorRoute(selectedRoute.value.id)
...
<template>
  <v-container class="operator-routes-view" fluid>
    ...
    <OperatorRouteCreateModal
      v-model="showCreateModal"
      :loading="actionLoading"
      :routes="availableRoutes"
      :routes-loading="availableRoutesLoading"
      @cancel="closeModals"
      @submit="handleCreateRoute"
    />

    <OperatorRouteStatusModal
